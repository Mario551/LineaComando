...
IF OBJECT_ID('dbo.per_comandos_registrados', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.per_comandos_registrados (
    ...
    CREATE INDEX idx_per_comandos_registrados_ruta
        ON dbo.per_comandos_registrados(ruta_comando);

    CREATE INDEX idx_per_comandos_registrados_activo
        ON dbo.per_comandos_registrados(activo);
END

-- Tabla: per_cola_comandos_estados
-- Catálogo de estados válidos para la cola de comandos

IF OBJECT_ID('dbo.per_cola_comandos_estados', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.per_cola_comandos_estados (
...
MERGE dbo.per_cola_comandos_estados AS destino
USING (VALUES
...
IF OBJECT_ID('dbo.per_cola_comandos', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.per_cola_comandos (
...
IF OBJECT_ID('dbo.per_cola_comandos_resultados', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.per_cola_comandos_resultados (
        ...
        CONSTRAINT fk_per_cola_comandos_resultados_comando
            ...
            REFERENCES dbo.per_cola_comandos(id)
            ON DELETE CASCADE,
    ...
    CREATE INDEX idx_per_cola_comandos_resultados_tipo_version
        ON dbo.per_cola_comandos_resultados(tipo, version_resultado);
END

GO

-- Función para obtener comandos pendientes (solo lectura)
CREATE OR ALTER FUNCTION dbo.obtener_comandos_pendientes(
    @tamanio_lote INT = 50,
...
CREATE OR ALTER PROCEDURE dbo.marcar_comandos_procesando
    @ids NVARCHAR(MAX)
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER PROCEDURE dbo.actualizar_fecha_leido
    @ids NVARCHAR(MAX)
AS
BEGIN
    SET NOCOUNT ON;
