...
internal sealed class InteractiveButtonReplyMessageInfobipConfiguracion
    : IEntityTypeConfiguration<InteractiveButtonReplyMessageInfobip>
{
    public void Configure(EntityTypeBuilder<InteractiveButtonReplyMessageInfobip> builder)
    {
        builder.ToTable("per_interactive_button_reply_messages_infobip");

        builder.HasKey(entity => entity.RecordId)
            .HasName("per_pk_interactive_button_reply_messages_infobip");
        ...
        builder.HasIndex(entity => entity.RecordIdInboundMessagesInfobip)
            .IsUnique()
            .HasDatabaseName("per_uk_interactive_button_reply_messages_inbound");

        builder.HasOne(entity => entity.InboundMessageInfobip)
            .WithOne(entity => entity.InteractiveButtonReplyMessageInfobip)
            .HasForeignKey<InteractiveButtonReplyMessageInfobip>(
                entity => entity.RecordIdInboundMessagesInfobip)
            .OnDelete(DeleteBehavior.Cascade)
            .HasConstraintName("per_fk_interactive_button_reply_messages_inbound");
    }
}
